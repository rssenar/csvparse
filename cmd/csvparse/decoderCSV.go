...
func (d *CSVDecoder) DecodeCSV(v interface{}) error {
	...
	csvRows, err := cp.GetCSVRows(d.file)
	if err != nil {
		return err
	}
	...
	if err := cp.CheckForDoubleHeaderNames(headerRow); err != nil {
		return err
	}

	// check interface type (v)
	// if type is not a slice, return error
	slice := cp.CheckInterfaceValue(v)
	...
	innerType := cp.GetInnerSliceType(v)
		...
		for j := 0; j < innerValueRow.Elem().NumField(); j++ {
			...
			case reflect.TypeOf(time.Now()):
				if _, ok := d.header[sFName]; ok {
					val := cp.ParseDate(csvRow[d.header[sFName]])
